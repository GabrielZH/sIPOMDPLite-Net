...
from data_processing import sa_tiger_grid_data_feeding
from pretrained_single_agent_model.network.qmdp_net import QMDPNet, QMDPNetPolicy
...
def run_training(params):
    ...
    train_feed = sa_tiger_grid_data_feeding.Datafeed(
        params,
        filename=datafile,
        mode="train",
        max_env=params.train_envs_proportion)
    valid_feed = sa_tiger_grid_data_feeding.Datafeed(
        params,
        filename=datafile,
        mode="valid",
...
def run_eval(params, modelfile):
    ...
    for _ in range(params.eval_trials):
        # build dataflows
        eval_feed = sa_tiger_grid_data_feeding.Datafeed(
            params,

...
from domains.tiger_grid.multi_agent.single_interaction_condition.learner_traj_evaluation import simulate_policy
...
class Datafeed(object):
    def __init__(
        ...
        self.domain = MATigerGridEnv(params)
...
class EvalDataFeed(dataflow.ProxyDataFlow):
    ...
    def eval_sample(
        ...
        nested_policy = self.db.nested_policies[sample_i[0]]
        db_step = self.db.steps[step_i[0]]

        init_state, init_action_i, init_action_j, _ = db_step
        ...
        results = np.zeros(
            [self.repeats * 2, 4],
            dtype=np.float32)
        processed_envs = np.stack([env, env], axis=1)
        processed_goals = self.task_param_processor.process_goals(goals)
        processed_init_belief = self.task_param_processor.process_beliefs(init_belief)
        processed_nested_policy = self.task_param_processor.process_nested_policies(nested_policy)
        ...
        for eval_i in range(self.repeats):
            ...
            learner_reward_sum = simulate_policy(
                policy=self.policy,
                env_map=env,
                processed_env_maps=processed_envs,
                init_belief=init_belief,
                processed_init_belief=processed_init_belief,
                goal=goal,
                processed_goals=processed_goals,
                nested_policy=nested_policy,
                processed_nested_policy=processed_nested_policy,
                params=self.domain.params,
                init_state=init_state,
                subj_init_action=init_action_i,
                obj_init_action=init_action_j)
...
class TrajDataFeed(dataflow.ProxyDataFlow):
    ...
    def process_samples(
        ...
        n_env_maps = np.stack([env_maps, env_maps], axis=1)
        n_goals = [goals, goals]
        n_goal_maps = self.task_param_processer.process_goals(n_goals)

        # Initial belief
        init_belief = self.db.beliefs[:][sample_i]
        init_belief = self.task_param_processer.process_beliefs(init_belief)

        # The opponent's policy (given by nested MDP)
        nested_policy = self.db.nested_policies[:][sample_i]
        nested_policy = self.task_param_processer.process_nested_policies(nested_policy)
        ...
        input_action_i = np.zeros(
            (self.step_size, self.batch_size),
            dtype=np.int32)
        input_action_i[valid_mask] = self.db.steps[:][step_idx_helper, 1]

        label_action_i = np.zeros(
            input_action_i.shape,
            dtype=np.int32)
        label_action_i[valid_mask] = self.db.steps[:][label_idx_helper, 1]

        input_action_j = np.zeros(
            (self.step_size, self.batch_size),
            dtype=np.int32)
        input_action_j[valid_mask] = self.db.steps[:][step_idx_helper, 2]

        label_action_j = np.zeros(
            input_action_j.shape,
            dtype=np.int32)
        label_action_j[valid_mask] = self.db.steps[:][label_idx_helper, 2]
        ...
        return [n_env_maps, n_goal_maps, init_belief, nested_policy,
                is_traj_head, input_action_i, input_action_j,
                input_observation,
                valid_step_mask, label_action_i, label_action_j]
...
class TaskParamPreprocessor(object):
    ...
    def process_goals(
        ...
        for goal in goals:
            goal_map = np.zeros(
                [goal.shape[0], self.domain.N, self.domain.M],
                dtype=np.int32)
            idx = np.unravel_index(
                goal, [self.domain.N, self.domain.M])
            goal_map[np.arange(goal.shape[0]), idx[0], idx[1]] = 1
            goal_maps.append(goal_map)
        goal_maps = np.stack(goal_maps, axis=1)
    ...
    def process_beliefs(
        ...
        """
        batch = beliefs.shape[0] if beliefs.ndim > 2 else 1
        num_agent = beliefs.shape[1] if beliefs.ndim > 2 else beliefs.shape[0]
    ...
    def process_nested_policies(
            self,
            nested_policies):
        """

        :param nested_policies
        """
        batch = nested_policies.shape[0] if nested_policies.ndim > 2 else 1
        nested_policies = nested_policies.reshape(
            [batch,
             self.domain.N,
             self.domain.M,
             self.domain.N,
             self.domain.M,
             self.domain.num_action])

        return nested_policies.astype(np.float32)

...
def simulate_policy(
        ...
        processed_goals,
        nested_policy,
        processed_nested_policy,
        params,
        init_state,
        subj_init_action=None,
        obj_init_action=None):
    """
    ...
    :param nested_policy
    :param processed_nested_policy
    :param params
    :param init_state:
    :param subj_init_action:
    :param obj_init_action:
    """
    if subj_init_action is None:
        subj_init_action = params.staynlisten
    if obj_init_action is None:
        obj_init_action = params.staynlisten
    ...
    policy.reset(
        ...
        nested_policy=processed_nested_policy)
    ...
    while True:
        ...
        if step_i == 0:
            # dummy first action
            expert_act_i = subj_init_action
            expert_act_j = obj_init_action
        else:
    ...
    while True:
        ...
        if step_i == 0:
            # dummy first action
            learner_act_i = subj_init_action
            learner_act_j = obj_init_action
        else:
            learner_act_i, _ = policy.output(
                input_subj_action=learner_act_i,
                input_obj_action=learner_act_j,
        ...
        if np.array_equal(learner_actions_i[-2:], [5, 5]) or \
                ...
                np.array_equal(learner_actions_i[-2:], [2, 4]) or \
                np.array_equal(learner_actions_i[-3:], [0, 0]):
            learner_failed = True
